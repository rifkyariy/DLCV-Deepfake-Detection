...
class SBI_Dataset(Dataset):
    ...
    def safe_resize(self, img, size):
        if img is None or img.size == 0 or img.shape[0] == 0 or img.shape[1] == 0:
            raise ValueError(f"[DATA ERROR] Empty image encountered before resize. Shape: {getattr(img, 'shape', None)}")
    ...
    def __getitem__(self, idx):
        ...
        while flag:
            try:
                ...
                img_f = self.safe_resize(img_f, self.image_size).astype('float32') / 255
                img_r = self.safe_resize(img_r, self.image_size).astype('float32') / 255
...
if __name__ == '__main__':
    ...
    # Determine the correct import path based on execution context
    current_file = os.path.abspath(__file__)
    
    # Check if we're in Docker (/app/) or local environment
    if '/app/' in current_file:
        # Docker environment - add /app/src/utils to path
        utils_path = '/app/src/utils'
        ...
        # Local environment - use relative imports
        try:
            ...
            from src.utils import blend as B
            from src.utils.initialize import init_ff
            from src.utils.funcs import IoUfrom2bboxes, crop_face, RandomDownScale
            if exist_bi:
                from src.utils.library.bi_online_generation import random_get_hull
...
if os.path.isfile('projects/DLCV/SBI/fsbi_src/utils/library/bi_online_generation.py'):
	sys.path.append('projects/DLCV/SBI/fsbi_src/utils/library/')
	# print('exist library')
	exist_bi=True
else:
	exist_bi=False
...
class ESBI_Dataset(Dataset):
	...
	def collate_fn(self,batch):
		...
		data={}
		# Convert to numpy arrays first, then to tensor to avoid numpy/torch compatibility issues
		img_r_np = np.array(img_r, dtype=np.float32)
		img_f_np = np.array(img_f, dtype=np.float32)
		data['img']=torch.cat([torch.from_numpy(img_r_np),torch.from_numpy(img_f_np)],0)
